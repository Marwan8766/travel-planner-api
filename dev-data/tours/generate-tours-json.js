...
// Read the JSON file
fs.readFile(
  './dev-data/tours/tours-json-exported-data.json',
  'utf8',
  (err, data) => {
    if (err) {
      console.error(err);
    ...
    // Loop over each tour
    tours.forEach((tour) => {
      // Check if the tour has startLocations with coordinates
      if (tour.startLocations && tour.startLocations.coordinates) {
        // Update the type property to "Point"
        tour.startLocations.type = 'Point';
      }

      // Check if the tour has locations
      if (tour.locations) {
    ...
    fs.writeFile(
      './dev-data/tours/updated_tours.json',
      updatedData,
      'utf8',
      (err) => {
        if (err) {
        ...
        console.log('Updated tours data saved to updated_tours.json');
      }
    );
  }
);
